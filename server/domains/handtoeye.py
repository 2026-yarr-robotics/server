"""Hand-to-eye domain: fixed-camera calibration and coordinate transforms."""
...
DEFAULT_HANDTOEYE_FILE = "T_base2camera.npy"


class HandToEyeDomain:
    """Manages hand-to-eye calibration for a fixed workspace camera."""
    ...
    def load_calibration(self) -> np.ndarray:
        return self._store.load(DEFAULT_HANDTOEYE_FILE)

    def save_calibration(self, matrix: np.ndarray) -> None:
        self._store.save(DEFAULT_HANDTOEYE_FILE, matrix)

    def get_calibration(self) -> dict[str, Any]:
        matrix = self.load_calibration()
        return {
            "file": DEFAULT_HANDTOEYE_FILE,
